...
| Command                | Description                                               | required flags |
|------------------------|-----------------------------------------------------------|----------------|
|```convertRegEx```      | convert the regular expression to an FSA                   |```-op```       |
...
|```cezarolimitdistribution```|Cezaro limit distribution through simulation|```-op, -c, (-sd), (-s)```|
|```estimationexpectedreward```|Estimation of expected reward by simulation|```-op, -c, -ns, (-sd)```|
|```estimationdistribution```|Estimation of distribution by simulation after specified number of steps|```-op, -c, -ns, (-sd)```|
|```estimationhittingstate```|Estimation of hitting state probabilities by simulation|```-op, -c, -s, (-sd), (-sa)```|
|```estimationhittingreward```|Estimation of cumulative reward hitting state by simulation|```-op, -c, -s, (-sd), (-sa)```|
|```estimationhittingstateset```|Estimation of hitting state set probabilities by simulation|```-op, -c, -ss, (-sd), (-sa)```|
|```estimationhittingrewardset```|Estimation of cumulative reward hitting state set probabilities by simulation|```-op, -c, -ss, (-sd), (-sa)```|
...
> The dataflow tool handles both Dataflow models and Max-Plus Matrix (MPM) models. To analyze either type, the command below can be run in the terminal. The ```model``` can have the extensions ```.sdf``` or ```.mpm```, representing Dataflow and MPM models respectively.
...
|```vectortracexml```|compute a vector trace for a state matrix or a set of state-space matrices as an XML file for the cmtrace tool (<https://github.com/Model-Based-Design-Lab/cmtrace>)|```-op, (-ni), (-is), (-sq)```|
...
|```scalesequence```|scale an event sequence, i.e., add a constant to all event time stamps; requires sequence and parameter|```-op, -pa, -sq````|
